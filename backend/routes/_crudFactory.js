...
function buildCrud({ table, fields, idPrefix = '', webhookPrefix = null, onCreated = null }) {
  ...
  router.post('/', requireWriter, async (req, res) => {
    try {
      ...
      if (typeof onCreated === 'function') {
        Promise.resolve(onCreated(r.rows[0], req)).catch((e) =>
          console.warn(`[crudFactory] onCreated failed for ${table}:`, e.message)
        );
      }
