...
export default function CrudPage({ title, subtitle, api, fields, statusKey, allowAttachments = true }) {
  ...
  const renderCell = (row, f) => {
    ...
    if (f.key === statusKey || ['status','priority','severity','threat_level','classification','tier','type','status_iucn'].includes(f.key)) {
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                  {attachList.map((a) => (
                    <li key={a.id} style={{
                      ...
                      <a
                        href={`${process.env.REACT_APP_API_BASE || 'http://localhost:3077/api'}/attachments/${a.id}/download`}
                        target="_blank"
